...
class DSLAccount(Structure):

	def __init__(self, env):
		r"""
		Initialize a DSLAccount.
		...
		if (self.allFields['Active'].present and \
			...
			self.dbConstraints += 'CONSTRAINT [CK_' + \
			  ...
			  '] = 0 OR [' + \
			  self.allFields['Installed'].dbName + '] = 1),\n'
		...
		if (self.allFields['TerminalID'].present and \
		    self.allFields['LCE'].present):
			self.dbConstraints += 'UNIQUE ([' + \
			  self.allFields['TerminalID'].dbName + '],[' + \
			  self.allFields['LCE'].dbName + ']),\n'

		if (self.allFields['TerminalID'].present and \
		    self.allFields['PairNumber'].present):
			self.dbConstraints += 'UNIQUE ([' + \
			  self.allFields['TerminalID'].dbName + '],[' + \
			  self.allFields['PairNumber'].dbName + ']),\n'
	...
	def getFormGuts(self, env, editable, arrangement, index):
		...
		out.append(self.getSingleItemRow('TestedSpeed', env, \
